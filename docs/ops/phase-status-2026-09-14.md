...
| **E GitHub `production` env** | **Optional** | Repo secret `BREVO_API_KEY` is enough; Environment only if you want approval gates |
...
1. GitHub secret **`BREVO_API_KEY`** = Brevo key **petralian.com** (not `.env.example` — public repo).
